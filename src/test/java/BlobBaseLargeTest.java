 ...
 *
 */
public class BlobBaseLargeTest
{

    private static String TEST_DIR = "./dbLargeTest/files";

    private static int loopCnt = 100000;

    public BlobBaseLargeTest()
    {
    }
    ...
    @Test(priority = 5)
    public void FileMapCheckNonExistenKeys() throws Exception
    {
        File root = new File(TEST_DIR);

        BlobBase map = BlobBase.getInstance(root);

        for (int i = loopCnt; i < loopCnt*2; i++)
        {
            File f = map.getFile(i);
            assertNull(f);            
        }
